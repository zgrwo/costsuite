...
- **AES-256 encryption** *(code-ready, not yet activated)*: Field-level encryption infrastructure (`AesEncryptionProvider`, AES-256-GCM with PBKDF2 key derivation, 100,000 iterations) is implemented and tested but **not registered in DI** — planned for activation in V2.0 when sensitive data classification is finalized.
