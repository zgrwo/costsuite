...
│
├── CLAUDE.md                               ← AI 协作行为准则
├── .claude/                                ← Claude Code 项目配置（gitignored）
│   └── exemptions.md                       ← 豁免决策登记
├── .gitignore
├── .editorconfig
...
| 文件 | 定位 |
|------|------|
| **CLAUDE.md** | AI 协作行为准则。不重复定义事实——通过任务路由表指引到上述文档 |
| **.claude/exemptions.md** | 豁免决策唯一登记处。待修 Bug → Sprint Backlog；已豁免 → 此处 |
| **.gitignore** | 排除规则。新增需要 gitignore 的文件类型时更新 |
