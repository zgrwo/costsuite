...
| # | 约束 | 影响范围 |
|---|------|---------|
| 1 | **离线模式仅限只读** — V1.0 不支持离线编辑与双向同步。网络断开时仅展示本地缓存的最近一次快照数据，禁止任何写入操作 → **v1.1 放宽**: SQLite 本地主库下编辑始终可用，离线仅指 ERP 同步暂停 | 同步服务、离线模式、UI 设计 |
| 2 | **Excel 2016/2019 为基准** — 365 动态数组作为增强 | UDF 设计、差异分析输出格式 |
...
│  技术: C# · FluentValidation · MediatR（可选事件总线）               │
├─────────────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────────────┤
│                      数据访问层 (DAL)                                │
│  SQLite(CRUD) · DuckDB(分析) · MemoryCache(L1) · Polly 重试        │
│  技术: Dapper · System.Data.SQLite · DuckDB.NET · DbUp             │
├─────────────────────────────────────────────────────────────────────┤
│                      基础设施层 (Infrastructure)                     │
│  NLog(脱敏) · CI/CD · 配置中心 · 审计日志(AOP) · AES-256 加密       │
│  技术: NLog · Microsoft.Extensions.DependencyInjection · DPAPI      │
└─────────────────────────────────────────────────────────────────────┘
```
...
public interface IVersionAdapter
{
    ...
    int GetDefaultReturnRowCount();         // 2016 需固定区域大小
}
```
...
- 差异摘要: 单单元格文本（Excel 2016）或结构化数组（Excel 365）
...
- **版本兼容**: 数组返回函数（`BOMEXPAND`, `VARIANCECHECK`）对 Excel 2016/2019 使用 `Ctrl+Shift+Enter` 模式，Excel 365 使用动态溢出。详见 [§13](./specification.md#13-excel-版本兼容矩阵)
...
var retryPolicy = Policy
    ...
    .Or<TimeoutException>()
    .Or<SqlException>()  // 数据库瞬态故障
    .WaitAndRetryAsync(
        retryCount: 3,
        sleepDurationProvider: retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), // 1s, 2s, 4s
        onRetry: (exception, timeSpan, retryCount, context) =>
        {
            _logger.Warn($"同步重试 #{retryCount}，等待 {timeSpan.TotalSeconds}s。错误: {exception.Message}");
        });
...
| 场景 | 算法 | 密钥管理 |
|------|------|---------|
| **传输** | TLS 1.2+ / HTTPS | 标准证书链 |
| **价格数据（静态）** | AES-256-CBC | Windows DPAPI（`ProtectedData.Protect`）保护数据加密密钥（DEK） |
| **密码** | BCrypt + Salt（work factor ≥ 12） | 无（单向哈希） |
| **SQLite 离线缓存** | SQLite Encryption Extension 或磁盘级 BitLocker | 系统级 |
...
| 功能 | Excel 2016 | Excel 2019 | Excel 365 | 适配策略 |
|------|-----------|-----------|-----------|---------|
| **BOM 展开** | `Ctrl+Shift+Enter` 数组公式 | 同 2016 | 动态数组自动溢出 | 检测版本，2016/2019 返回固定区域 + 提示 |
| **差异分析** | 单单元格返回文本摘要 | 同 2016 | 返回结构化数组 | UDF 内部判断 `Application.Version` |
| **仪表盘** | 独立 WPF 窗口 | 同 2016 | 同 2016 | 无差异 |
| **任务窗格** | 支持 | 支持 | 支持 | 无差异 |
| **Ribbon** | 支持 | 支持 | 支持 | 无差异 |
...
| **开发者可见错误** | NLog 记录 + CorrelationId 关联 | "同步超时: ERP 端点无响应 (CorrelationId: abc-123)" |
| **静默错误** | NLog 记录，不打扰用户 | "后台重试已耗尽 (3/3)，将在 5 分钟后恢复" |

### 14.2 NLog 配置

```xml
<!-- 分级采样 -->
<logger name="*" minlevel="Error" writeTo="file-target" />
<logger name="*" minlevel="Warn" writeTo="remote-target" />
<!-- Info 1% 采样避免日志洪泛 -->
<logger name="*" minlevel="Info" writeTo="sampled-file" 
        final="true">
  <filters>
    <when condition="random(100) > 1" action="Ignore" />
  </filters>
</logger>
<!-- 敏感字段脱敏规则 -->
<parameter name="password" layout="${event-properties:password}" 
           replaceWith="***MASKED***" />
<parameter name="price" layout="${event-properties:price}" 
...
> 📋 **配对文档**: 实施路线图、Sprint 计划、风险管理请参见 [plan.md](./plan.md)
