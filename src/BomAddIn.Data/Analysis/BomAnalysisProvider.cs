    ...
    public class BomAnalysisProvider : IBomAnalysisProvider, IDisposable
    {
            ...
            lock (_lock)
            {
                ...
                cmd.CommandText = @"
                    ...
                    CREATE TABLE BomClosure (
                        AncestorId BIGINT, DescendantId BIGINT,
                        Depth INTEGER, PathQuantity DOUBLE
                    );
                ";
                cmd.ExecuteNonQuery();
                ...
                try
                {
                    ...
                    LoadTableBatched(sqliteConn, "BomClosure",
                        "SELECT AncestorId, DescendantId, Depth, PathQuantity FROM BomClosure");
                }
                catch
                {
                ...
                indexCmd.CommandText = @"
                    ...
                    CREATE INDEX IF NOT EXISTS idx_closure_ancestor ON BomClosure(AncestorId, Depth);
                    CREATE INDEX IF NOT EXISTS idx_closure_descendant ON BomClosure(DescendantId, Depth);
                ";
        ...
        public List<BomExpandedNode> ExpandBomViaClosure(string itemCode, DateTime? asOfDate = null)
        {
            lock (_lock)
            {
                ...
                // 检查 Closure Table 是否有数据
                using (var checkCmd = _duckDb.CreateCommand())
                {
                    checkCmd.CommandText = "SELECT COUNT(*) FROM BomClosure LIMIT 1";
                ...
                // Step 2: Closure Table 单次查询获取所有后代（限制 maxDepth=20 与 BFS 对齐）
                const int maxDepth = 20;
                bool hasDeeper = false;

                using (var closureCmd = _duckDb.CreateCommand())
                {
                    closureCmd.CommandText = @"
                        SELECT c.DescendantId, c.Depth, c.PathQuantity,
                               m.Code, m.Name, m.Unit, m.Category
                        FROM BomClosure c
                        JOIN Materials m ON c.DescendantId = m.Id
                        WHERE c.AncestorId = $1 AND c.Depth <= $2
                        ORDER BY c.Depth, c.DescendantId";
                    closureCmd.Parameters.Add(new DuckDBParameter(rootId));
                    closureCmd.Parameters.Add(new DuckDBParameter(maxDepth));

                    using var reader = closureCmd.ExecuteReader();
                    while (reader.Read())
                    {
                        results.Add(new BomExpandedNode
                        {
                            ...
                            ParentMaterialId = null, // Closure Table 不保留直接父级信息
                            ItemCode = reader.GetString(3),
                            Description = reader.GetString(4),
                            Quantity = Convert.ToDouble(reader.GetValue(2)),
                ...
                // 检测是否存在超过 maxDepth 的更深层节点
                using (var depthCmd = _duckDb.CreateCommand())
                {
                    depthCmd.CommandText = "SELECT COUNT(*) FROM BomClosure WHERE AncestorId = $1 AND Depth > $2";
                    depthCmd.Parameters.Add(new DuckDBParameter(rootId));
                    depthCmd.Parameters.Add(new DuckDBParameter(maxDepth));
                    hasDeeper = Convert.ToInt64(depthCmd.ExecuteScalar()) > 0;
                }

                // 与 BFS 路径对齐：深度超限时记录警告 + 插入截断哨兵节点
                if (hasDeeper)
                {
                    Infrastructure.Logging.AppLogger.Warn(
                        $"BOM \"{itemCode}\" Closure 展开达到深度上限 ({maxDepth} 层)，结果可能不完整。",
                        typeof(BomAnalysisProvider));
                    ...
                    results.Add(new BomExpandedNode
                    {
                        ...
                        ParentMaterialId = null,
                        ItemCode = "[TRUNCATED]",
                        Description = $"BOM depth exceeded {maxDepth} levels. Results incomplete.",
                        Quantity = 0,
                        Unit = "",
                        Source = "System",
                        VersionState = ""
